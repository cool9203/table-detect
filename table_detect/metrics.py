...
def remove_dotted_lines(complete_stats: np.ndarray) -> np.ndarray:
    ...
    for idx in prange(complete_stats.shape[0]):
        ...
        if y_middle - prev_y_middle <= 2:
            ...
            x1_area, y1_area, x2_area, y2_area = min(x, x1_area), min(y, y1_area), max(x + w, x2_area), max(y + h, y2_area)
            width_area += w
            area_count += 1
...
@njit("Tuple((uint8[:,:],int32[:,:]))(uint8[:,:],int32[:,:],int32[:,:],float64)", fastmath=True, cache=True, parallel=False)
...
def compute_char_length(thresh: np.ndarray) -> Tuple[Optional[float], Optional[np.ndarray], Optional[np.ndarray]]:
    ...
    if len(relevant_stats) > 0:
        ...
        characters_thresh, chars_array = create_character_thresh(
            thresh=thresh, stats=relevant_stats, discarded_stats=discarded_stats, char_length=char_length
        )
...
def compute_median_line_sep(
    ...
    # Get contours cells
    contours_cells = [Cell(x1=x, y1=y, x2=x + w, y2=y + h) for idx, (x, y, w, h) in enumerate(stats_contours) if idx > 0]
